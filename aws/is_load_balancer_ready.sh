...
# Expects:
# 1 --> NAME (Required): The name of the load balancer instance we are checking.
# 2 --> TIMEOUT, in minutes (Optional, default: 5)

header "IS LOAD BALANCER READY?"
...
do
    ...
    load_balancers_json=$(aws elbv2 describe-load-balancers --filter Name=instance-state-code,Values="16")
...
function is_load_balancer_ready() {
...
if [[ "$ready" == true ]]; then
    footer "IS LOAD BALANCER READY?" "YES"
    exit 0
fi 

footer "IS LOAD BALANCER READY?" "NO"
exit 666